...
double U[max_no_variable][points][points],F1[max_no_variable][points][points], flux_out[max_no_variable][points][points],flux_out2[max_no_variable][points][points],bx[points][points],Bz[points][points],
...
double pressure[points][points],density[points][points],velocity_y[points][points],velocity_x[points][points],velocity[points][points],magnetic[points][points],velocity_z[points][points],
       ethalpy[points][points],x[points],U_new2[max_no_variable][points][points];
...
double lamda_max1[points][points],lamda_max2,min_lamda_3,t=0,tf=0.5,delta_y,y[points],lamda_max4,afven_speed_y[points][points],slow_speed_y[points][points],fast_speed_y[points][points],lamda_max3[points][points];

double  NetdivBx[points][points], NetdivBy[points][points];
double divBy[points][points], divBx[points][points], Bp[points][points];
...
double lamda_max, CFL=.65;

int nj,ni,n1,n2,n3,n4,n5,n6;
...
FILE *density_file, *velocity_file, *pressure_file,*magnetic_file,*velocity_y_file,*magnetic3_file,*magneticz_file;
    printf("\n2-D ORSZAG_TANG VORTEX problem with WENO-RUSANOV\n\n");


    density_file  = fopen("density.dat","w");
    velocity_file = fopen("velocity_z.dat","w");
  ...
  velocity_y_file = fopen("velocity_y.dat","w");
   magnetic3_file = fopen("magnetic_x.dat","w");
    magneticz_file = fopen("magnetic_z.dat","w"); 
...
void initilization()
{
 ...
 j=0;
   for(k=6; k<=size-6; k=k+2)                     //Here i for y axis, j for x-axis
    {
  ...
  for(i=6; i<=size-6; i=i+2)                     //Here i for y axis, j for x-axis
    {
       
       
	   for(j=6; j<=size-6; j=j+2)
       {
	     pressure[i][j] = 1/GAMMA;
	
	    density[i][j] = 1; 	         
    
	    magnetic[i][j] = sin(4*3.142*x[j])/GAMMA; 
  	
	    velocity_x[i][j] = -sin(2*3.142*y[i]);
      
	    velocity_y[i][j] = sin(2*3.142*x[j]);
	 ...
	 ////////////////////////////////////////////////////////////////////////////////
	 for(i=6; i<=size-6; i=i+2)                                    //need to be checked 
    {

     for(j=6; j<=size-6; j=j+2)                                    //need to be checked 
    {
   ...
   for(j=6; j<=size-6; j=j+2)
    {
  		...
  		pressure[ni][j]     =   pressure[8][j];
        density[ni][j]     =   density[8][j];
        ethalpy[ni][j]      =  ethalpy[8][j];
        velocity_x[ni][j]    =  velocity_x[8][j];
		velocity_y[ni][j]    =  velocity_y[8][j];
		velocity_z[ni][j]    = 	velocity_z[8][j];
          magnetic[ni][j]    =    magnetic[8][j];
          Bz[ni][j]          =    Bz[8][j] ;
          bx[ni][j]          =  bx[8][j] ;
	    speed_sound[ni][j]=     speed_sound[8][j];           
   ...
   for(j=6; j<=size-6; j=j+2)
    {
		...
		pressure[4][j]     =   pressure[n2][j];
        density[4][j]     =   density[n2][j];
        ethalpy[4][j]      =  ethalpy[n2][j];
        velocity_x[4][j]    =  velocity_x[n2][j]  ;
		velocity_y[4][j]    =  velocity_y[n2][j] ;
		velocity_z[4][j]    = 	velocity_z[n2][j] ;
          magnetic[4][j]    =    magnetic[n2][j];
          Bz[4][j]          =    Bz[n2][j]  ;
           bx[4][j]          =  bx[n2][j]  ;
	    speed_sound[4][j]=     speed_sound[n2][j];           
    //
...
for(i=6; i<=size-6; i=i+2)
    {
        ...
        pressure[i][4]     =   pressure[i][n1];
        density[i][4]     =   density[i][n1];
        ethalpy[i][4]      =  ethalpy[i][n1];
        velocity_x[i][4]    =  velocity_x[i][n1];  
		velocity_y[i][4]    =  velocity_y[i][n1] ;
		velocity_z[i][4]    = 	velocity_z[i][n1]; 
          magnetic[i][4]    =    magnetic[i][n1];
          Bz[i][4]          =    Bz[i][n1];  
           bx[i][4]          =  bx[i][n1] ; 
	    speed_sound[i][4]=     speed_sound[i][n1];           
...
for(i=6; i<=size-6; i=i+2)
    {
    	 	pressure[i][nj]     =    pressure[i][8];
        density[i][nj]      =    density[i][8];
        ethalpy[i][nj]      =    ethalpy[i][8];
        velocity_x[i][nj]   =   velocity_x[i][8]  ;
		velocity_y[i][nj]   =   velocity_y[i][8] ;
		velocity_z[i][nj]   = 	 velocity_z[i][8] ;
          magnetic[i][nj]   =   magnetic[i][8];
          Bz[i][nj]         =   Bz[i][8]  ;
           bx[i][nj]        =  bx[i][8]  ;
	   speed_sound[i][nj]   =  speed_sound[i][8] ;          
   ...
   for(j=6; j<=size-6; j=j+2)
    {
   		...
   		pressure[n3][j]     =   pressure[10][j];
        density[n3][j]     =   density[10][j];
        ethalpy[n3][j]      =  ethalpy[10][j];
        velocity_x[n3][j]    =  velocity_x[10][j];
		velocity_y[n3][j]    =  velocity_y[10][j];
		velocity_z[n3][j]    = 	velocity_z[10][j];
          magnetic[n3][j]    =    magnetic[10][j];
          Bz[n3][j]          =    Bz[10][j] ;
          bx[n3][j]          =  bx[10][j] ;
	    speed_sound[n3][j]=     speed_sound[10][j];           
   ...
   for(j=6; j<=size-6; j=j+2)
    {
		...
		pressure[2][j]     =   pressure[n5][j];
        density[2][j]     =   density[n5][j];
        ethalpy[2][j]      =  ethalpy[n5][j];
        velocity_x[2][j]    =  velocity_x[n5][j]  ;
		velocity_y[2][j]    =  velocity_y[n5][j] ;
		velocity_z[2][j]    = 	velocity_z[n5][j] ;
          magnetic[2][j]    =    magnetic[n5][j];
          Bz[2][j]          =    Bz[n5][j]  ;
           bx[2][j]          =  bx[n5][j]  ;
	    speed_sound[2][j]=     speed_sound[n5][j];           
    //
...
for(i=6; i<=size-6; i=i+2)
    {
		...
		pressure[i][2]     =   pressure[i][n5];
        density[i][2]     =   density[i][n5];
        ethalpy[i][2]      =  ethalpy[i][n5];
        velocity_x[i][2]    =  velocity_x[i][n5];  
		velocity_y[i][2]    =  velocity_y[i][n5] ;
		velocity_z[i][2]    = 	velocity_z[i][n5]; 
          magnetic[i][2]    =    magnetic[i][n5];
          Bz[i][2]          =    Bz[i][n5];  
           bx[i][2]          =  bx[i][n5] ; 
	    speed_sound[i][2]=     speed_sound[i][n5];           
...
for(i=6; i<=size-6; i=i+2)
    {
	 	...
	 	pressure[i][n3]     =    pressure[i][10];
        density[i][n3]      =    density[i][10];
        ethalpy[i][n3]      =    ethalpy[i][10];
        velocity_x[i][n3]   =   velocity_x[i][10]  ;
		velocity_y[i][n3]   =   velocity_y[i][10] ;
		velocity_z[i][n3]   = 	 velocity_z[i][10] ;
          magnetic[i][n3]   =   magnetic[i][10];
          Bz[i][n3]         =   Bz[i][10]  ;
           bx[i][n3]        =  bx[i][10]  ;
	   speed_sound[i][n3]   =  speed_sound[i][10] ;          
   ...
   for(j=6; j<=size-6; j=j+2)
    {
		...
		pressure[n4][j]     =   pressure[12][j];
        density[n4][j]     =   density[12][j];
        ethalpy[n4][j]      =  ethalpy[12][j];
        velocity_x[n4][j]    =  velocity_x[12][j];
		velocity_y[n4][j]    =  velocity_y[12][j];
		velocity_z[n4][j]    = 	velocity_z[12][j];
          magnetic[n4][j]    =    magnetic[12][j];
          Bz[n4][j]          =    Bz[12][j] ;
          bx[n4][j]          =  bx[12][j] ;
	    speed_sound[n4][j]=     speed_sound[12][j];           
...
for(j=6; j<=size-6; j=j+2)
    {
		...
		pressure[0][j]     =   pressure[n6][j];
        density[0][j]     =   density[n6][j];
        ethalpy[0][j]      =  ethalpy[n6][j];
        velocity_x[0][j]    =  velocity_x[n6][j]  ;
		velocity_y[0][j]    =  velocity_y[n6][j] ;
		velocity_z[0][j]    = 	velocity_z[n6][j] ;
          magnetic[0][j]    =    magnetic[n6][j];
          Bz[0][j]          =    Bz[n6][j]  ;
           bx[0][j]          =  bx[n6][j]  ;
	    speed_sound[0][j]=     speed_sound[n6][j];           
    //
...
for(i=6; i<=size-6; i=i+2)
    {
		...
		pressure[i][0]     =   pressure[i][n6];
        density[i][0]     =   density[i][n6];
        ethalpy[i][0]      =  ethalpy[i][n6];
        velocity_x[i][0]    =  velocity_x[i][n6];  
		velocity_y[i][0]    =  velocity_y[i][n6] ;
		velocity_z[i][0]    = 	velocity_z[i][n6]; 
          magnetic[i][0]    =    magnetic[i][n6];
          Bz[i][0]          =    Bz[i][n6];  
           bx[i][0]          =  bx[i][n6] ; 
	    speed_sound[i][0]=     speed_sound[i][n6];           
...
for(i=6; i<=size-6; i=i+2)
    {
	 	...
	 	pressure[i][n4]     =    pressure[i][12];
        density[i][n4]      =    density[i][12];
        ethalpy[i][n4]      =    ethalpy[i][12];
        velocity_x[i][n4]   =   velocity_x[i][12]  ;
		velocity_y[i][n4]   =   velocity_y[i][12] ;
		velocity_z[i][n4]   = 	 velocity_z[i][12] ;
          magnetic[i][n4]   =   magnetic[i][12];
          Bz[i][n4]         =   Bz[i][12]  ;
           bx[i][n4]        =  bx[i][12]  ;
	   speed_sound[i][n4]   =  speed_sound[i][12] ;          
...
for(j=4; j<=size-4; j=j+2)
   {
    afven_speed_x[i][j]      =  (bx[i][j])/(sqrt(density[i][j]));
 	...
 	afven_speed[i][j]     = (sqrt((bx[i][j]*bx[i][j])+(magnetic[i][j]*magnetic[i][j]))/sqrt(density[i][j]));
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 if(lamda_max3[i][j]<fabs(lamda2[k][i][j]))
  ...
  for(i=4; i<=size-6; i=i+2)
{

for(j=4; j<=size-6; j=j+2)
{
  ...
  for(j=4; j<=size-6; j=j+2)
    {
       ...
       F1[0][i][j+1] = (density[i][j]*velocity_x[i][j] + density[i][j+2]*velocity_x[i][j+2])*0.5 - .5*(UL1[0][i][j+1]);
        ...
        F1[5][i][j+1] =((magnetic[i][j]*velocity_x[i][j])+(magnetic[i][j+2]*velocity_x[i][j+2]))*mu*.5 - ((bx[i][j]*velocity_y[i][j])+(bx[i][j+2]*velocity_y[i][j+2]))*.5-.5*(UL1[5][i][j+1]);  

        G1[7][i+1][j] =((bx[i][j]*velocity_y[i][j])+(bx[i+2][j]*velocity_y[i+2][j]))*mu*.5 - ((magnetic[i][j]*velocity_x[i][j])+(magnetic[i+2][j]*velocity_x[i+2][j]))*.5-.5*(UR1[7][i+1][j]);  

	  
 

        F1[6][i][j+1] = ((Bz[i][j]*velocity_x[i][j])+(Bz[i][j+2]*velocity_x[i][j+2]))*mu*.5 - ((bx[i][j]*velocity_z[i][j])+(bx[i][j+2]*velocity_z[i][j+2]))*.5-.5*(UL1[6][i][j+1]);

        G1[6][i+1][j] = ((Bz[i][i]*velocity_y[i][j])+(Bz[i+2][j]*velocity_y[i+2][j]))*mu*.5 - ((magnetic[i][j]*velocity_z[i][j])+(magnetic[i+2][j]*velocity_z[i+2][j]))*.5-.5*(UR1[6][i+1][j]);
...
void RK3()
{
 ...
 //periodic();
    ...
    for(i=4; i<=size-8; i=i+2)
    {
 for(j=4; j<=size-8; j=j+2)
    {
   ...
   for(j=6; j<=size-6; j=j+2)
    {
		...
		pressure[ni][j]     =   pressure[6][j];
        density[ni][j]     =   density[6][j];
        ethalpy[ni][j]      =  ethalpy[6][j];
        velocity_x[ni][j]    =  velocity_x[6][j];
		velocity_y[ni][j]    =  velocity_y[6][j];
		velocity_z[ni][j]    = 	velocity_z[6][j];
          magnetic[ni][j]    =    magnetic[6][j];
          Bz[ni][j]          =    Bz[6][j] ;
          bx[ni][j]          =  bx[6][j] ;
	    speed_sound[ni][j]=     speed_sound[6][j];           
 ...
 for(i=6; i<=size-6; i=i+2)
    {
	 	...
	 	pressure[i][nj]     =    pressure[i][6];
        density[i][nj]      =    density[i][6];
        ethalpy[i][nj]      =    ethalpy[i][6];
        velocity_x[i][nj]   =   velocity_x[i][6]  ;
		velocity_y[i][nj]   =   velocity_y[i][6] ;
		velocity_z[i][nj]   = 	 velocity_z[i][6] ;
          magnetic[i][nj]   =   magnetic[i][6];
          Bz[i][nj]         =   Bz[i][6]  ;
           bx[i][nj]        =  bx[i][6]  ;
	   speed_sound[i][nj]   =  speed_sound[i][6] ;          
...
for(j=4; j<=size-6; j=j+2)
    {
	    ...
	    U_L[3][i][j+1]=density_L[i][j+1]*velocity_z[i][j+1];
      
	    U_L[4][i][j+1]= (pressure_L[i][j+1]/(GAMMA-1)) + (0.5*density_L[i][j+1]*(velocity_xL[i][j+1]*velocity_xL[i][j+1]+velocity_yL[i][j+1]*velocity_yL[i][j+1]+velocity_z[i][j+1]*velocity_z[i][j+1]))+(0.5*mu*(Bz[i][j+1]*Bz[i][j+1] + magnetic_L[i][j+1]*magnetic_L[i][j+1]+bx_L[i][j+1]*bx_L[i][j+1]));
      
	    U_L[5][i][j+1]= magnetic_L[i][j+1];
      
	    U_L[6][i][j+1]= Bz[i][j+1];
	    ...
	    U_R[3][i][j-1]=density_R[i][j-1]*velocity_z[i][j-1];
      
	    U_R[4][i][j-1]= (pressure_R[i][j-1]/(GAMMA-1)) + (0.5*density_R[i][j-1]*(velocity_xR[i][j-1]*velocity_xR[i][j-1]+velocity_yR[i][j-1]*velocity_yR[i][j-1]+velocity_z[i][j-1]*velocity_z[i][j-1]))+(0.5*mu*(Bz[i][j-1]*Bz[i][j-1] + magnetic_R[i][j-1]*magnetic_R[i][j-1]+bx_R[i][j-1]*bx_R[i][j-1]));
      
	    U_R[5][i][j-1]= magnetic_R[i][j-1];
      
	    U_R[6][i][j-1]= Bz[i][j-1];
...
////////////////////////////////////////////////////////////////////// y-direction state variables //////////////////////////
	    ...
	    G_L[3][i+1][j]= d_L[i+1][j]*velocity_z[i][j];
      
	    G_L[4][i+1][j]= (p_L[i+1][j]/(GAMMA-1)) + (0.5*d_L[i+1][j]*(v_xL[i+1][j]*v_xL[i+1][j]+v_yL[i+1][j]*v_yL[i+1][j]+velocity_z[i][j]*velocity_z[i][j]))+(0.5*mu*(Bz[i+1][j]*Bz[i+1][j] + BY_L[i+1][j]*BY_L[i+1][j]+BX_L[i+1][j]*BX_L[i+1][j]));
      
	    G_L[5][i+1][j]= BY_L[i+1][j];
      
	    G_L[6][i+1][j]= Bz[i+1][j];
	    ...
	    G_R[3][i-1][j]=  d_R[i-1][j]*velocity_z[i][j];
      
	    G_R[4][i-1][j]= (p_R[i-1][j]/(GAMMA-1)) + (0.5*d_R[i-1][j]*(v_xR[i-1][j]*v_xR[i-1][j]+v_yR[i-1][j]*v_yR[i-1][j]+velocity_z[i][j]*velocity_z[i][j]))+(0.5*mu*(Bz[i-1][j]*Bz[i-1][j] + BY_R[i-1][j]*BY_R[i-1][j]+BX_R[i-1][j]*BX_R[i-1][j]));
      
	    G_R[5][i-1][j]= BY_R[i-1][j];
      
	    G_R[6][i-1][j]= Bz[i-1][j];
       ...
       F_W[0][i][j+1] = (density_R[i][j+1]*velocity_xR[i][j+1] + density_L[i][j+1]*velocity_xL[i][j+1])*0.5 - .5*(UL2[0][i][j+1]);
		...
		G_W[0][i+1][j] = (d_L[i+1][j]*v_yL[i+1][j] + d_R[i+1][j]*v_yR[i+1][j])*0.5 - .5*(UR2[0][i+1][j]);
	 	...
	 	F_W[1][i][j+1] = (density_R[i][j+1]*velocity_xR[i][j+1]*velocity_xR[i][j+1] + density_L[i][j+1]*velocity_xL[i][j+1]*velocity_xL[i][j+1])*0.5 
                  ...
                  +(.5*(pressure_R[i][j+1]+pressure_L[i][j+1])+.25*mu*(((magnetic_L[i][j+1]*magnetic_L[i][j+1])+(bx_L[i][j+1]*bx_L[i][j+1]))+(magnetic_R[i][j+1]*magnetic_R[i][j+1])+(bx_R[i][j+1]*bx_R[i][j+1])+(Bz[i][j]*Bz[i][j])+(Bz[i][j+2]*Bz[i][j+2]))) 
	    ...
	    G_W[2][i+1][j] = (d_L[i+1][j]*v_yL[i+1][j]*v_yL[i+1][j] + d_R[i+1][j]*v_yR[i+1][j]*v_yR[i+1][j])*0.5 
                  ...
                  +(.5*(p_L[i+1][j]+p_R[i+1][j])+.25*mu*(((BY_L[i+1][j]*BY_L[i+1][j])+(BX_L[i+1][j]*BX_L[i+1][j])+(Bz[i][j]*Bz[i][j]))+(BY_R[i+1][j]*BY_R[i+1][j])+(BX_R[i+1][j]*BX_R[i+1][j])+(Bz[i+2][j]*Bz[i+2][j]))) 
        ...
        F_W[2][i][j+1] = ((density_L[i][j+1]*velocity_xL[i][j+1]*velocity_yL[i][j+1])+(density_R[i][j+1]*velocity_xR[i][j+1]*velocity_yR[i][j+1]))*.5 
         ...
         F_W[3][i][j+1] = ((density_L[i][j+1]*velocity_xL[i][j+1]*velocity_z[i][j+2])+(density_R[i][j+1]*velocity_xR[i][j+1]*velocity_z[i][j]))*.5 
                               -((Bz[i][j]*bx_L[i][j+1])+(Bz[i][j+2]*bx_R[i][j+1]))*mu*.5-.5*(UL2[3][i][j+1]);

        
	    G_W[3][i+1][j] = ((d_L[i+1][j]*v_yL[i+1][j]*velocity_z[i+2][j])+(d_R[i+1][j]*v_yR[i+1][j]*velocity_z[i][j]))*.5 
                               -((Bz[i][j]*BY_L[i+1][j])+(Bz[i+2][j]*BY_R[i+1][j]))*mu*.5-.5*(UR2[3][i+1][j]);
        ...
        F_W[6][i][j+1] = ((Bz[i][j]*velocity_xL[i][j+1])+(Bz[i][j]*velocity_xL[i][j+1]))*mu*.5 - ((bx_R[i][j+1]*velocity_z[i][j])+(bx_L[i][j+1]*velocity_z[i][j+2]))*.5-.5*(UL2[6][i][j+1]);

        G_W[6][i+1][j] = ((Bz[i][i]*v_yL[i][j+1])+(Bz[i+2][j]*v_yR[i+1][j]))*mu*.5 - ((BY_L[i+1][j]*velocity_z[i][j])+(BY_R[i+1][j]*velocity_z[i+2][j]))*.5-.5*(UR2[6][i+1][j]);
	    ...
	    F_W[4][i][j+1] = ((density_L[i][j+1]*velocity_xL[i][j+1]*ethalpy_L[i][j+1]) + (density_R[i][j+1]*
                      velocity_xR[i][j+1]*ethalpy_R[i][j+1]))*.5- (.5*mu*(bx_L[i][j+1]*(bx_L[i][j+1]*velocity_xL[i][j+1]+(magnetic_L[i][j+1]*velocity_yL[i][j+1])+(Bz[i][j]*velocity_z[i][j]))+(bx_R[i][j+1]*(bx_R[i][j+1]*velocity_xR[i][j+1]+(magnetic_R[i][j+1]*velocity_yR[i][j+1])+(Bz[i][j+2]*velocity_z[i][j+2])))))-.5*(UL2[4][i][j+1]); 

  
        G_W[4][i+1][j] = ((d_L[i+1][j]*v_yL[i+1][j]*e_L[i+1][j]) + (d_R[i+1][j]*
                        v_yR[i+1][j]*e_R[i+1][j]))*.5- (.5*mu*(BY_L[i+1][j]*(BX_L[i+1][j]*v_xL[i+1][j]+(BY_L[i+1][j]*v_yL[i+1][j])+(Bz[i][j]*velocity_z[i][j]))+(BY_R[i+1][j]*(BX_R[i+1][j]*v_xR[i+1][j]+(BY_R[i+1][j]*v_yR[i+1][j])+(Bz[i+2][j]*velocity_z[i+2][j])))))-.5*(UR2[4][i+1][j]); 
  ...
  for(j=4; j<=size-6; j=j+2)
    {
	  ...
	  for(k=0;k<8;k++)
      {
        ...
        F1[k][i][j+1] = W*F1[k][i][j+1]+  (1-W)*F_W[k][i][j+1];

       G1[k][i+1][j]  = W*G1[k][i+1][j] + (1-W)*G_W[k][i+1][j];
...
void source()
{
 ...
 for(i=4; i<=size-6; i=i+2)       // need to be checked // some assumptions ? is small though
    {
   for(j=4; j<=size-6; j=j+2)
 ...
 for(i=4; i<=size-8; i=i+2)       // need to be checked // some assumptions ? is small though
    {
   for(j=4; j<=size-8; j=j+2)
