...
double lamda_max1[points][points],lamda_max2,min_lamda_3,t=0,tf=0.801,delta_y,y[points],lamda_max4,afven_speed_y[points][points],slow_speed_y[points][points],fast_speed_y[points][points],lamda_max3[points][points];

double  NetdivBx[points][points], NetdivBy[points][points], divBy[points][points], divBx[points][points], B[points][points],divByx[points][points], divBxy[points][points],divBzy[points][points], divBzx[points][points], NetdivBz[points][points],div_B[points][points],TBflux[points][points];
...
double lamda_max, CFL=.65, r,f,r_0=0.1,r_1=0.115;
...
FILE *density_file, *velocity_file, *pressure_file,*magnetic_file,*velocity_y_file,*magnetic3_file,*magneticz_file;
    printf("\n2-D RIEMANN PROBLEM WITH WENO-RUSANOV\n\n");
  ...
  velocity_y_file = fopen("velocity_y.dat","w");
   magnetic3_file = fopen("magnetic_x.dat","w");
    magneticz_file = fopen("magnetic_t.dat","w"); 
...
for(j=6; j<=size-6; j=j+2)
    {
  		...
  		pressure[ni][j]      =   pressure[size-6][j];
        density[ni][j]       =   density[size-6][j];
        ethalpy[ni][j]       =  ethalpy[size-6][j];
        velocity_x[ni][j]    =  velocity_x[size-6][j];
		velocity_y[ni][j]    =  velocity_y[size-6][j];
		velocity_z[ni][j]    = 	velocity_z[size-6][j];
          magnetic[ni][j]    =    magnetic[size-6][j];
          Bz[ni][j]          =    Bz[size-6][j] ;
          bx[ni][j]          =  bx[size-6][j] ;
	    speed_sound[ni][j]   =  speed_sound[size-6][j];           
...
for(i=6; i<=size-6; i=i+2)
    {
        ...
        pressure[i][size-4]      =   pressure[i][size-6];
        density[i][size-4]       =   density[i][size-6];
        ethalpy[i][size-4]       =  ethalpy[i][size-6];
        velocity_x[i][size-4]    =  velocity_x[i][size-6];  
		velocity_y[i][size-4]    =  velocity_y[i][size-6] ;
		velocity_z[i][size-4]    = 	velocity_z[i][size-6]; 
          magnetic[i][size-4]    =    magnetic[i][size-6];
          Bz[i][size-4]          =    Bz[i][size-6];  
           bx[i][size-4]         =  bx[i][size-6] ; 
	    speed_sound[i][size-4]   =     speed_sound[i][size-6];           
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////// for j-2  and size-2 cells
   ...
   for(j=6; j<=size-6; j=j+2)                    ///n3=size-2....//
    {
...
if(count<(NetdivBx[i][j]+NetdivBx[i][j]))
count=(NetdivBx[i][j]+NetdivBx[i][j]);
  ...
  if(int (t)%2==0)
cout<<"TIME\t"<<t<<"\tDIV.ERROR\t"<<count<<endl;
...
/////////////////////////////////////////////////////////////////// for Ghost cells next to the boundary............///////
	  ...
	  for(j=6; j<=size-6; j=j+2)
    {
  		...
  		pressure[size-6][j]     =   pressure[size-8][j];
        density[size-6][j]     =   density[size-8][j];
        ethalpy[size-6][j]      =  ethalpy[size-8][j];
        velocity_x[size-6][j]    =  velocity_x[size-8][j];
		velocity_y[size-6][j]    =  velocity_y[size-8][j];
		velocity_z[size-6][j]    = 	velocity_z[size-8][j];
          magnetic[size-6][j]    =    magnetic[size-8][j];
          Bz[size-6][j]          =    Bz[size-8][j] ;
          bx[size-6][j]          =  bx[size-8][j] ;
	    speed_sound[size-6][j]=     speed_sound[size-8][j];           
